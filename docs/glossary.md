...
<dt id="csv">CSV</dt>
<dd>
  A CSV (Comma Separated Values) file is a text file where each line (row)
  contains data (columns) separated by commas. It's a useful and largely
  universal format for getting data into or out of a spreadsheet. You can
  register new students using one or two CSVs exported from your institution's
  online learning system or student records. The server has a utility for
  combining two such CSVs into one: see <a href="registering-users/spreadsheet#helpful-utility-for-creating-the-csv-file">utility
  for creating user CSV file</a>.
</dd>
