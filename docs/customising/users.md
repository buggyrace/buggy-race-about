...
| `IS_USER_AUTO_ADMIN_TINTED` | The admin interface will automatically tint users' buttons in some places based on some hard-coded criteria: for example, if the user has comments. Tints assigned by admin always override auto settings. Set this to `No` if you don't want this behaviour.  <br/><br/> _Default value:_ `Yes` |
