...
| `BUGGY_EDITOR_REPO_NAME` | This should match the name in the `BUGGY_EDITOR_REPO_URL` and is used in some of the GitHub API calls: if you've forked the repo and not changed its name, you won't need to change this. If `IS_USING_VCS` is `No`, this setting is ignored. If you've changed this setting and you're using a VCS (like Git), see also `BUGGY_EDITOR_DIR_NAME` in the Editor group of config settings, which should probably be the same: it's the name of the directory the student's repo is cloned into.  <br/><br/> _Default value:_ `buggy-race-editor` |
